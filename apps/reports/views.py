...
def _serialize_booking_for_list(booking):
    ...
    slots = [
        item.slot.label
        for item in sorted(items, key=lambda i: i.slot.time)
        if item.slot
    ]
    return {
        'id':             booking.id,
...
def dashboard_stats(request):
    ...
    Returns stat cards + today's and tomorrow's booking lists.
    """
    today    = date.today()
    ...
    # Trend = % change vs yesterday (avoid division by zero)
...
def revenue_chart(request):
    """
    GET /api/v1/admin/reports/revenue-chart/?days=7
    Returns daily revenue for bar chart.
    """
    try:
...
def activity_breakdown(request):
    """
    GET /api/v1/admin/reports/activity-breakdown/
    Returns per-activity booking counts for pie chart.
    """
    rows = (
        BookingItem.objects
        ...
        .order_by('-count')[:6]   # top 6 for pie chart
    )
...
def export_bookings_csv(request):
    ...
    writer.writerow([
        ...
        'Activity', 'Date', 'Time Slot', 'Persons', 'Amount', 'Status',
    ])
