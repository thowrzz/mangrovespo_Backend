...
def _fire_emails(booking_id: int):
    try:
        ...
        logger.warning("Could not queue confirmation emails for booking %s: %s", booking_id, exc)
...
def verify_payment(request):
    ...
    try:
        booking = Booking.objects.select_for_update().get(razorpay_order_id=razorpay_order_id)
    ...
    try:
        ...
        if rz_amount_paise != expected_paise:
            logger.error("Amount mismatch for order %s", razorpay_order_id)
        ...
        return Response({"error": "Could not verify amount with Razorpay"}, status=status.HTTP_502_BAD_GATEWAY)
    ...
    with transaction.atomic():
        ...
        if booking.status != "confirmed":
            ...
            booking.save(update_fields=["razorpay_payment_id", "status", "payment_verified_at"])
...
def razorpay_webhook(request):
    ...
    try:
        ...
        logger.warning("Webhook: no booking for order %s", order_id)
        return Response(status=status.HTTP_200_OK)
